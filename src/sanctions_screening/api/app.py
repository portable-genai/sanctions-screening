"""FastAPI application for Sanctions Screening Copilot (G2).
...
``SANCTIONS_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``SANCTIONS_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real screening decision.
...
from .schemas import HealthResponse, ScreenRequest, ScreenResponse
...
_INSECURE_DEMO_ENV = "SANCTIONS_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "SANCTIONS_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "SANCTIONS_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "SANCTIONS_S2S_AUDIENCE"
_API_HOST_ENV = "SANCTIONS_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``SANCTIONS_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``SANCTIONS_PROFILE`` must never be allowed to
    ...
    ``SANCTIONS_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "SANCTIONS_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "sanctions-screening"
...
_APP_TITLE = "Sanctions Screening Copilot"
...
#
# Note what is NOT in this expression: SANCTIONS_S2S_TOKEN. A service
...
@app.post("/v1/screen", response_model=ScreenResponse, tags=["artifacts"])
def screen(
    ...
    result = service.screen(request.to_domain(), actor=principal.actor, tenant=principal.tenant)
    # The hand-off never fails an already-scored, already-audited screening; the response says
    ...
    return ScreenResponse.from_domain(
