"""Local OwnershipGraphPort: replay a captured cdd-sow-research UBO fixture through the shared
reader.

The fixture is a body captured from cdd-sow-research's FROZEN contract and shipped in-package. It is
read by the SAME ``parse_ubo_graph`` the managed adapter uses over the wire, so the offline family
and the managed family cannot read the contract two different ways: a drift that would break
cdd-sow-research in production breaks the consumer contract test here first.
"""
...
#: The tenant the captured cdd-sow-research graph belongs to, as the contract publishes it. Named
#: here so the
...
def load_fixture_graph() -> OwnershipGraph:
    """Read and parse the shipped cdd-sow-research UBO fixture (also used by the contract test)."""
...
class LocalOwnershipGraphAdapter:
    """Return the captured cdd-sow-research graph for any subject, so screening exercises the owner
    path.
    """
    ...
    def resolve(
        ...
        # The graph's own data tag decides, not the id the caller named. cdd-sow-research publishes
        # the
