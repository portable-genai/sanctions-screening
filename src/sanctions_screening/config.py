...
One env var (``SANCTIONS_PROFILE``) selects the adapter family for every
...
``${VAR:-default}`` default, SET-AND-EMPTY RAISES rather than inheriting that default or
resolving to the empty string (an operator who emptied a value expressed an intent, it names
nothing, and the loader is the last place that still knows a default was written), and
SET-AND-VALID wins. That raise happens at import, in the deploy, naming the variable and its
documented default, which is where a configuration error made seconds earlier belongs.
"""
...
#: the ``X-Answered-By`` the local narration adapter notes when it drafts. One constant so the
...
class ProfileChoice:
    ...
    The variable is ``SANCTIONS_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("SANCTIONS_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "ownership_graph": {
        "local": f"{_PKG}.adapters.local.ownership_graph:LocalOwnershipGraphAdapter",
        "gcp": f"{_PKG}.adapters.gcp.ownership_graph:Doc1OwnershipGraphAdapter",
        "onprem": f"{_PKG}.adapters.onprem.ownership_graph:OnPremOwnershipGraphAdapter",
    },
    "adverse_media": {
        "local": f"{_PKG}.adapters.local.adverse_media:LocalAdverseMediaAdapter",
    ...
    "narration": {
        ...
        "onprem": f"{_PKG}.adapters.onprem.narration:OnPremNarrationAdapter",
    },
}
...
def _expand(value: str) -> str:
    ...
    Delegates to :func:`.envread.setting_or_default`, so there is exactly one implementation of
    the rule: the default written in the file is the UNSET answer, and an EMPTIED variable
    raises rather than resolving to the empty string. ``${VAR}`` with no ``:-`` is the same
    call with an empty default, so unset yields ``""`` and emptied still refuses.
    """
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``SANCTIONS_SETTINGS``) that does not
...
class Settings:
    ...
    #: default. UNSET arrives as ``""``; SET-AND-EMPTY no longer arrives at all, because the
    #: expansion refuses it at boot rather than handing this field an empty audience for the
    #: life of the process. Empty means the adapter can
    ...
    #: default policy; a deployment overrides one threshold without restating the rest.
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            doc1_a2a_url=str(data.get("doc1_a2a_url") or ""),
...
class Container:
    ...
    @cached_property
    def ownership_graph(self) -> OwnershipGraphPort:
        adapter = self._bind("ownership_graph")
        assert isinstance(adapter, OwnershipGraphPort)
        return adapter
