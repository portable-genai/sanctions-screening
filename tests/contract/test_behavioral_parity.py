...
from sanctions_screening.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    demo.
    """
    service = build_screening_service(build_container(local_settings()))
    result = service.screen(sample_cases.PII_CASE, actor=sample_cases.ACTOR)
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_screening_service(local).screen(
    ...
    with pytest.raises(NotImplementedError):
        build_screening_service(onprem).screen(
            sample_cases.CONFIRMED_CASE, actor=sample_cases.ACTOR
        )
