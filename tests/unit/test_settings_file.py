...
def test_an_env_var_set_to_empty_does_not_inherit_the_files_default(
    ...
    """Three-state expansion: unset takes the default, EMPTIED refuses, a set value wins.

    The emptied half used to resolve to the empty string. It now raises: ``${VAR:-default}`` is
    ``setting_or_default(VAR, default)`` one layer down and must answer the same way, and the
    loader is the only place that still knows a default was written. Resolving to empty made
    ``${VAR:-http://audit:8080}`` with ``VAR=""`` indistinguishable from ``${VAR:-}``, and for a
    URL, an allowlist or a path the empty string is the permissive branch downstream.
    """
...
def test_a_bare_env_reference_still_refuses_when_emptied(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """``${VAR}`` with no ``:-`` is the same call with an empty default, not a two-state read."""
    path = tmp_path / "settings.yaml"
    path.write_text("region: ${TEMPLATE_TEST_REGION}\n", encoding="utf-8")

    monkeypatch.delenv("TEMPLATE_TEST_REGION", raising=False)
    assert _read_settings_file(path)["region"] == ""
