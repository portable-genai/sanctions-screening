...
from sanctions_screening.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from sanctions_screening.ports.identity import (
    VERIFIED,
...
_API_MODULE = "sanctions_screening.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        "  ownership_graph:",
        *[
            f"    {p}: {_PKG}.adapters.local.ownership_graph:LocalOwnershipGraphAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.ownership_graph:OnPremOwnershipGraphAdapter",
        "  adverse_media:",
        *[
            f"    {p}: {_PKG}.adapters.local.adverse_media:LocalAdverseMediaAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.adverse_media:OnPremAdverseMediaAdapter",
        "  narration:",
        *[
            f"    {p}: {_PKG}.adapters.local.narration:LocalNarrationAdapter"
            for p in ("local", "gcp")
        ],
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str) -> Any:
    ...
    supplies it. An empty ``audience`` argument means UNCONFIGURED, which is expressed by
    UNSETTING the variable: the loader now refuses an EMPTIED one at boot rather than resolving
    it to the empty string, and ``test_an_emptied_audience_refuses_at_boot`` pins that.
    """
...
def test_an_emptied_audience_refuses_at_boot(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    """An EMPTIED audience is an expressed intent that names no resource, so it never boots.

    Unset means nobody configured the audience and every end-user route answers 503 naming the
    variable (the test above). Emptied is different: somebody set it and it names nothing, and
    the settings loader is the last place that still knows a default was written, so it raises
    there rather than handing the adapter an empty audience that would 503 on every request for
    ...
    monkeypatch.delenv(_AUDIENCE_ENV, raising=False)
    assert Settings.load(path).iap_audience == ""
