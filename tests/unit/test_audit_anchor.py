...
from sanctions_screening.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "SANCTIONS_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for case in (sample_cases.CONFIRMED_CASE, sample_cases.CLEAN_CASE, sample_cases.PII_CASE):
        service.screen(case, actor=sample_cases.ACTOR)
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        build_screening_service(build_container(settings)).screen(
            sample_cases.CONFIRMED_CASE, actor=sample_cases.ACTOR
        )
