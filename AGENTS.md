...
Sanctions Screening Copilot (G2): Deterministic sanctions and PEP screening copilot with cited, human-reviewed dispositions.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `sanctions_screening`, environment prefix
`SANCTIONS`, region `asia-southeast1`.
...
- `src/sanctions_screening/domain/` is PURE stdlib. `kernel.py` holds the vertical-neutral
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `SANCTIONS_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `SANCTIONS_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `SANCTIONS_PROFILE` (mentioning it in a refusal message is wanted, reading
