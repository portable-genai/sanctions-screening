...
drives the actual screening service, the actual deterministic match engine, the actual
...
* **Nothing is faked.** No stub service, no pre-baked JSON. The match bands, the audit records,
...
from sanctions_screening.domain import (
    kernel,
    models,
)
...
from sanctions_screening.service_factory import (
...
SERVICE_NAME = "Sanctions Screening Copilot"
CATALOG_ID = "G2"
REPOSITORY = "sanctions-screening"
...
#: A clean name: no list carries it. Still routed, because the system never clears itself.
ROUTINE_CASE = models.ScreeningRequest(
...
ESCALATING_CASE = models.ScreeningRequest(
    ...
    kind=models.PartyKind.ENTITY,
    subject_id="acme",
    context="high-value transfer flagged from host 2001:db8::7, mail ops@volkov.example",
)

#: A confirmed individual carrying a planted national id, for the redact-before-anything proof.
PII_CASE = models.ScreeningRequest(
    subject="Dmitri Volkov (FICTIONAL)",
    kind=models.PartyKind.INDIVIDUAL,
    dob="1968-04-11",
    context=(
        "urgent: NRIC " + PLANTED_NRIC + " and mail ops@volkov.example seen on host 2001:db8::7"
    ),
)
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="clean",
        label="A clean screen: no match, but STILL routed for a human decision",
        narration=(
            "A name no list carries. The band comes from deterministic code, never from a "
            "model. The engine proposes a false positive, and it is STILL routed: the system "
            "never clears a match on its own, so even a clean screen is a human decision."
        ),
    ),
    Step(
        key="confirmed",
    ...
    Step(
        ...
        narration=(
            "The same path, with a national id planted in the case context. The identifier is "
    ...
    Step(
        ...
        narration=(
            ...
            "returned successfully would convert a disposition into an unreviewed decision."
        ),
    ),
)
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.service = build_screening_service(self.container)
    ...
    def _step_clean(self) -> Produced:
        return self._screen_panels(ROUTINE_CASE, expect_confirmed=False)

    def _step_confirmed(self) -> Produced:
        return self._screen_panels(ESCALATING_CASE, expect_confirmed=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._screen_panels(PII_CASE, expect_confirmed=True)
        ...
        panels.append(
            Panel(
                ...
                rows=(
                    Row("Identifier in the submitted context", PLANTED_NRIC, "warn"),
                    Row(
    ...
    def _step_review_queue(self) -> Produced:
        ...
        queue = Panel(
            ...
            note=(
                ...
                "a buffered disposition is never mistaken for a reviewed one."
            ),
        )
        findings = Panel(
            title="Findings",
            rows=(
                Row("Dispositions routed", str(self.routed)),
                Row("Confirmed matches (dual control)", str(self.true_matches)),
                Row(
            ...
            note=(
                "Every disposition is accounted for. A flag with no routing reference is "
        ...
        actions = Panel(
            title="Next actions",
            rows=(
                Row("Reviewer", "open a queued item and approve or reject the disposition"),
    ...
    def _screen_panels(self, case: models.ScreeningRequest, *, expect_confirmed: bool) -> Produced:
        result = self.service.screen(case, actor=ACTOR, tenant=FIXTURE_TENANT)
        self.cases += 1
        review_ref = self.container.review_router.route(result, maker=ACTOR, tenant=TENANT)
        self.routed += 1
        confirmed = result.band is models.MatchBand.CONFIRMED
        if confirmed:
            self.true_matches += 1
        consistent = confirmed == expect_confirmed and bool(review_ref)
        match_rows = [
            Row(
                s.party.name + " -> " + s.matches[0].entry.list_id,
                s.matches[0].entry.entry_id + " @ " + str(s.matches[0].confidence),
                "bad" if s.band is models.MatchBand.CONFIRMED else "warn",
            )
            for s in result.party_screenings
            if s.matches
        ]
        decision = Panel(
            title="Disposition: " + result.subject,
            rows=(
                Row("Band", result.band.value, "bad" if confirmed else "ok"),
                Row("Recommendation", result.recommendation.value),
                Row("Severity", result.severity.value),
                Row("Owners screened", str(result.owners_screened)),
            ...
            note=(
                "The band is computed by pure stdlib code and is replayable. A model narrates "
                "the memo; it never produces the band."
            ),
            tone="ok" if consistent else "bad",
        )
        matches = Panel(
            title="Match arithmetic",
            rows=tuple(match_rows) or (Row("matches", "none at or above the review band", "ok"),),
            note="Every match names the list entry and the confidence the engine computed.",
        )
        evidence = Panel(
            title="Evidence",
            rows=tuple(Row(c.title, c.snippet or c.source_id) for c in result.citations)
        ...
        return [decision, matches, evidence], facts
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            redacted_summary="Volkov Metals OJSC (FICTIONAL): screened confirmed",
        )
    )


def _exit_review(container: Any) -> Any:
    citation = kernel.Citation(source_id="list:UN:UN-1001", title="UN listing", snippet="")
    result = models.ScreeningResult(
        ...
        ownership=None,
        adverse_media=(),
        band=models.MatchBand.CONFIRMED,
        recommendation=models.Recommendation.TRUE_MATCH,
        memo=models.DispositionMemo(text="Confirmed.", grounded=True),
        severity=kernel.Severity.CRITICAL,
        decision=kernel.Decision.ESCALATED,
        summary="Volkov Metals OJSC (FICTIONAL): screened confirmed",
        requires_human_review=True,
        citations=(citation,),
    )
    return container.review_router.route(result, maker=ACTOR, tenant=TENANT)
...
def _exit_ownership(container: Any) -> Any:
    return container.ownership_graph.resolve("acme", ESCALATING_CASE.subject, tenant=FIXTURE_TENANT)


def _exit_adverse(container: Any) -> Any:
    return container.adverse_media.search("Dmitri Volkov (FICTIONAL)")
