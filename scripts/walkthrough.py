...
def _check_clean(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("confirmed"):
        problems.append("a clean name banded confirmed")
    # Every disposition routes, including a clean one: the system never clears a match itself.
    if not facts.get("requires_human_review"):
        problems.append("a clean screen did not require human review")
    if not facts.get("review_ref"):
        problems.append("rule R8: a clean disposition was not routed")
    return problems


def _check_confirmed(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if not facts.get("confirmed"):
        problems.append("a designated party did not band confirmed")
    if not facts.get("review_ref"):
        problems.append("rule R8: a confirmed match was flagged but not routed")
    if int(facts.get("owners_screened", 0)) < 1:
        problems.append("no beneficial owners were screened for the designated party")
    totals = state["totals"]
    if totals["routed"] != totals["cases"]:
        problems.append("routed and screened counts disagree")
    return problems
...
def _check_redaction(state: dict[str, Any]) -> list[str]:
    ...
    if not facts.get("review_ref"):
        problems.append("rule R8: the case with personal data was not routed")
    return problems
